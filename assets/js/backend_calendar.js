...
var BackendCalendar = {
    ...
    record_Ids:{},
    record_type:this.FILTER_TYPE_SERVICE,
        ...
        if (GlobalVariables.availableProviders.length > 0) {
            ...
            */
            ...
            $.each(GlobalVariables.availableProviders, function(index, provider) {
                ...
                optgroupHtml += '<label> <input name="chk_proviers" type="checkbox" value="' + provider['id'] + '" '
                    + 'type_="' + BackendCalendar.FILTER_TYPE_PROVIDER + '" '
                    + 'google-sync="' + hasGoogleSync + '"/>'
                    + provider['first_name'] + ' ' + provider['last_name']
                    + ' </label>';
            });
            $('#providers-list').html(optgroupHtml);
        }
    ...
    bindEventHandlers: function() {
         ...
         */
        $('#btn_chooseProvider').click(function(){
            var provider_ids=[];
            $('input[name=chk_proviers]').each(function( ){
                if($(this).prop('checked') == true){
                    provider_ids.push($(this).prop('value'));
                }
            });
            if(provider_ids.length ==0)
                return;
            BackendCalendar.record_type = BackendCalendar.FILTER_TYPE_PROVIDER;
            BackendCalendar.record_Ids = provider_ids;
            BackendCalendar.refreshCalendarAppointments_Providers(
                $('#calendar'),
                provider_ids,
                BackendCalendar.FILTER_TYPE_PROVIDER,
        ...
        $('#select-filter-item').change(function() {
            ...
            BackendCalendar.record_type = BackendCalendar.FILTER_TYPE_SERVICE;
         ...
         *
         */
        $('#export-schedule').click(function(e) {

            var postUrl = GlobalVariables.baseUrl + '/index.php/backend_api/ajax_export_schedule';

            var postData = {
                'csrfToken': GlobalVariables.csrfToken,
                'record_id' : $('#select-filter-item').val(),
                'filter_type': $('#select-filter-item option:selected').attr('type'),
                'start_date':$('#calendar').fullCalendar('getView').visStart,
                'end_ate':$('#calendar').fullCalendar('getView').visEnd
            };

            $.post(postUrl, postData, function(response) {
                /////////////////////////////////////////////////////////
                //console.log('Export Schedule url :', response);
                /////////////////////////////////////////////////////////
    ...
    refreshCalendarAppointments_Providers: function($calendar, recordIds, filterType, startDate, endDate) {
        var postUrl = GlobalVariables.baseUrl + '/index.php/backend_api/ajax_get_calendar_appointments_providers';
        var postData = {
            'csrfToken': GlobalVariables.csrfToken,
        ...
        $.post(postUrl, postData, function(response) {
            ...
            if (filterType === BackendCalendar.FILTER_TYPE_PROVIDER && calendarView !== 'month') {
                ...
                $.each(GlobalVariables.availableProviders, function(index, provider) {
                    if ($.inArray(provider['id'] ,recordIds) != -1) {
                        var workingPlan = jQuery.parseJSON(provider.settings.working_plan);
                        var unavailablePeriod;
    ...
    calendarViewDisplay : function(view) {
        ...
        if(BackendCalendar.record_type == BackendCalendar.FILTER_TYPE_SERVICE)
        {
            BackendCalendar.refreshCalendarAppointments(
                $('#calendar'),
                $('#select-filter-item').val(),
