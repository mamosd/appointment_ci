...
class Backend_api extends CI_Controller {
    ...
    public function ajax_get_calendar_appointments_providers() {
        try {
            if ($this->privileges[PRIV_APPOINTMENTS]['view'] == FALSE) {
                throw new Exception('You do not have the required privileges for this task.');
            }
            ...
            // Get appointments
            $where_clause = $where_id . ' in (' . implode(",",$_POST['record_ids']) . ')' ;
            ...
            // Get unavailable periods (only for provider).
            if ($_POST['filter_type'] == FILTER_TYPE_PROVIDER) {
                $where_clause = $where_id . ' in (' . implode(",",$_POST['record_ids']) . ') AND is_unavailable = TRUE';
    ...
    public function ajax_filter_customers() {
    	try {
            ...
            foreach($customers as &$customer) {
                ...
                $invoices = $this->invoices_model->get_batch(array('id_users_customer' => $customer['id']));
     ...
     * [AJAX] Delete an unavailable time period from database.
     *
     * @param numeric Record id to be deleted.
     */
    public function ajax_export_schedule() {
        try {
            ...
            */
            ...
            $where_clause = array(
                $where_id => $_POST['record_id'],
                //'start_datetime >=' => $_POST['start_date'],                                                  //Do we need start and end datetime?
                //'end_datetime <=' => $_POST['end_date'],
                'is_unavailable' => FALSE
            );

            $res['appointments'] = $this->appointments_model->get_batch($where_clause);
            $output[] = array('Start date','End date','Booked By','Type of Service','Cost of Service');         //implement multi-language here.
            foreach($res['appointments'] as &$appointment) {
                $appointment['provider'] = $this->providers_model->get_row($appointment['id_users_provider']);
                $appointment['service'] = $this->services_model->get_row($appointment['id_services']);
                $appointment['customer'] = $this->customers_model->get_row($appointment['id_users_customer']);

                $tmp['start_datetime'] = $appointment['start_datetime'];
                $tmp['end_datetime'] = $appointment['end_datetime'];
                $tmp['booked_by']=$appointment['customer']['first_name']. ' ' . $appointment['customer']['last_name'];
                $tmp['type']=$appointment['service']['name'];
                $tmp['cost']=$appointment['service']['price'];
                $output[] = $tmp;
            }
            $tmp = time();
            $filename = FCPATH . '/uploads/' . $tmp.'.csv';
            $file_link = $this->config->item('base_url') . '/uploads/' . $tmp.'.csv';
    ...
    public function ajax_export_schedule_providers() {
        try {
            /*            if ($this->privileges[PRIV_APPOINTMENTS]['delete'] == FALSE) {
                            throw new Exception('You do not have the required privileges for this task.');
                        }
            */
            ...
            $where_clause = 'id_users_provider in ('.implode(",",$_POST['record_ids']).') and is_unavailable = FALSE';
            $res['appointments'] = $this->appointments_model->get_batch($where_clause);
            $output[] = array('Start date','End date','Booked By','Type of Service','Cost of Service');         //implement multi-language here.
            foreach($res['appointments'] as &$appointment) {
                $appointment['provider'] = $this->providers_model->get_row($appointment['id_users_provider']);
    ...
    /**
     * [AJAX] Save appointment checked.
     */
    public function ajax_save_appointment_checked() {
        try {
            $this->load->model('appointments_model');
            ...
            $appointment['is_shown'] = $is_shown;
            $appointment['is_invoice'] = $is_invoice;
    ...
    public function ajax_create_invoice() {
        try {
            ...
            $this->load->model('customers_model');
            $this->load->model('services_model');
            $id_users_customer = $_POST['id_users_customer'];
            $sendEmail = $_POST['sendEmail'];
            //Get invoices checked, make file and send email
            $where_clause = array(
                'id_users_customer' => $id_users_customer,
                'is_invoice'=> 1
            );
            $invoice_appointments = $this->appointments_model->get_batch($where_clause);
            $customer = $this->customers_model->get_row($id_users_customer);
            $tmp = time();
            $filename = FCPATH . '/uploads/invoice_' . $customer['first_name'] . ' ' .$customer['last_name'].$tmp.'.pdf';
            $file_link = $this->config->item('base_url') .'/uploads/invoice_' . $customer['first_name'] . ' ' .$customer['last_name'].$tmp.'.pdf';
            error_reporting(E_ALL);
            ini_set('display_errors',true);
...
// instantiate and use the dompdf class
            ...
            $html = "
            <div>
            <strong>Invoice To</strong><br/>$customer[first_name] $customer[last_name]
            ...
            ";
            $cost_total = 0;
            foreach($invoice_appointments as $appointment) {
                $service = $this->services_model->get_row($appointment['id_services']);
                $cost_total += $service['price'];
                $html .="<tr style='border:1px black solid ;'><td>$appointment[book_datetime]</td><td>$appointment[notes]</td><td>$service[price]</td></tr>";
            }
            $html .="<tr><td></td><td>SubTotal</td><td>$cost_total</td></tr>";
            $html .="</table></div>";
...
fwrite($fp,$output);
            ...
            if(isset($sendEmail) && $sendEmail == "true")
            {
                $this->load->library('email');

                $this->email->from('admin@edge360sporttherapy.com', 'Your Name');
                $this->email->to('keiespro@gmail.com');//$customer['email'];

                $this->email->subject('Invoice');
                $this->email->attach($filename, 'attachment', 'invoice.pdf', 'application/pdf');
                $this->email->message('Please check the attached invoice document.');
                if(!$this->email->send())
                {
            ...
            //now save invoice info in database
            $ret = $this->invoices_model->add(array('id_users_customer'=>$id_users_customer,'filename'=>$customer['last_name'].$tmp.'.pdf','file_link'=>$file_link));

            echo json_encode($this->invoices_model->get_row($ret));
        } catch(Exception $exc) {
            echo json_encode(array(
                'exceptions' => array(exceptionToJavaScript($exc))
            ));
